...
if(isset($_SESSION['user_id'])){
	...
	if($get_current_lesson_id != ""){
		...
		if($get_current_comment_id != ""){
			...
			/*- Header ----------------------------------------------------------- */
			$website_title = "$get_current_lesson_title - $l_delete_comment";
			if(file_exists("./favicon.ico")){ $root = "."; }
			elseif(file_exists("../favicon.ico")){ $root = ".."; }
			elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
			elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
			...
			if($get_current_comment_user_id == "$my_user_id" OR $get_my_user_rank == "admin" OR $get_my_user_rank == "moderator"){
				if($process == "1"){
					...
					$result = mysqli_query($link, "DELETE FROM $t_courses_lessons_comments WHERE comment_id=$get_current_comment_id") or die(mysqli_error($link));

					// Header
					$url = "$root/$get_current_course_title_clean/$get_current_module_title_clean/$get_current_lesson_title_clean.php?course_id=$get_current_course_id&module_id=$get_current_module_id&lesson_id=$get_current_lesson_id&l=$l&ft=success&fm=comment_deleted#comments";
					header("Location: $url");
					exit;
				...
				<!-- You are here -->
					...
					&gt;
					<a href=\"delete_comment_to_lesson.php?course_id=$get_current_course_id&amp;module_id=$get_current_module_id&amp;lesson_id=$get_current_lesson_id&amp;comment_id=$get_current_comment_id&amp;l=$l\">$l_delete_comment</a>
					</p>
				...
				<!-- Delete comment form -->
					...
					<p>
					<a href=\"delete_comment_to_lesson.php?course_id=$get_current_course_id&amp;module_id=$get_current_module_id&amp;lesson_id=$get_current_lesson_id&amp;comment_id=$get_current_comment_id&amp;l=$l&amp;process=1\" class=\"btn_warning\">$l_confirm_delete</a>
					</p>
				<!-- //Delete comment form -->
				";
			} // access
