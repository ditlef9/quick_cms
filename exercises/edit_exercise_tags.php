...
/**
*
* File: exercise/edit_exercise_tags.php
* Version 1.0.0
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_exercise_id == ""){
		...
		if($action == ""){
			if($process == 1){
				...
				if($get_tag_id != ""){
					...
					$url = "edit_exercise_tags.php?exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
					$url = $url . "&ft=$ft&fm=$fm";
					header("Location: $url");
					exit;
			...
			&gt;
			<a href=\"$root/exercises/edit_exercise_tags.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l\">$l_tags</a>
			</p>
			<!-- //Where am I? -->
			...
			<form method=\"post\" action=\"edit_exercise_tags.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
			...
			<!-- Tags -->
			<p style=\"padding-bottom:0;margin-bottom:0;\"><b>$l_tags:</b></p>
			<table>";
				...
				echo"
				 <tr>
				  ...
				  <td style=\"padding: 4px 4px 3px 0px;vertical-align:top;\">
					 <span><a href=\"edit_exercise_tags.php?exercise_id=$exercise_id&amp;action=delete_tag&amp;tag_id=$get_tag_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l&amp;process=1\"><img src=\"_gfx/icons/delete.png\" alt=\"delete.png\" /></a></span>
				  </td>
				 </tr>";
			}
			echo"
		...
		elseif($action == "delete_tag"){
			...
			$query = "SELECT tag_id, tag_text, tag_text, tag_clean FROM $t_exercise_index_tags WHERE tag_id=$tag_id_mysql AND tag_exercise_id=$get_exercise_id";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
