...
/**
*
* File: exercise/edit_exercise_images.php
* Version 1.0.0
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	$query = "SELECT muscle_group_id, muscle_group_name_clean FROM $t_muscle_groups WHERE muscle_group_id='$get_exercise_muscle_group_id_main'";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	...
	if($get_exercise_id == ""){
		...
		if($action == "upload_image"){
			if($process == "1"){
				...
				if($file_type == "jpg" OR $file_type == "jpeg" OR $file_type == "png" OR $file_type == "gif"){
					...
					if(move_uploaded_file($_FILES['inp_image']['tmp_name'], $target_path)) {
						...
						if(is_numeric($width) && is_numeric($height)){
							...
							if($width < 889){
								...
								$url = $url . "&ft=error&fm=width_have_to_be_bigger&width=$width&height=$height";
								header("Location: $url");
								exit;
							}
							if($height < 889){
								unlink($target_path);
								$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
								$url = $url . "&ft=error&fm=height_have_to_be_bigger&width=$width&height=$height";
								header("Location: $url");
								exit;
							}
							...
							// Header
							$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
							$url = $url . "&ft=success&fm=image_uploaded&file_name=$inp_exercise_image_file&file_path=$inp_exercise_image_path";
							header("Location: $url");
							exit;
							...
							$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
							$url = $url . "&ft=error&fm=file_is_not_an_image";
							header("Location: $url");
							exit;
						}
					}
   						...
   						switch ($_FILES['inp_image'] ['error']){
							case 1:
								$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
								$url = $url . "&ft=error&fm=to_big_file";
								header("Location: $url");
								exit;
								break;
							case 2:
							...
							case 3:
								$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
								$url = $url . "&ft=error&fm=only_parts_uploaded";
								header("Location: $url");
								exit;
								break;
							case 4:
								$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
								$url = $url . "&ft=error&fm=no_file_uploaded";
								header("Location: $url");
								exit;
								break;
						}
					...
					$url = "edit_exercise_images.php?action=$action&exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
					$url = $url . "&ft=error&fm=invalid_file_type&file_type=$file_type";
					header("Location: $url");
					exit;
				}
			...
			&gt;
			<a href=\"$root/exercises/edit_exercise_images.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l\">$l_images</a>
			</p>
			<!-- //Where am I? -->
			...
			<form method=\"post\" action=\"edit_exercise_images.php?action=$action&amp;exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
		...
		elseif($action == ""){
			...
			<a href=\"$root/exercises/my_exercises.php?main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l\">$l_my_exercises</a>
			&gt;
			<a href=\"$root/exercises/edit_exercise.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l\">$l_edit</a>
			&gt;
			<a href=\"$root/exercises/edit_exercise_images.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l\">$l_images</a>
			</p>
			<!-- //Where am I? -->
			...
			<p>
			<a href=\"$root/exercises/edit_exercise_images.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l&amp;action=upload_image\" class=\"btn\">$l_upload_image</a>
			</p>
