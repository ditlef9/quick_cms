...
if(isset($_GET['l'])) {
	...
	$query = "SELECT workout_session_main_id FROM $t_workout_plans_sessions_main ORDER BY workout_session_main_id DESC LIMIT 0,1";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	list($get_workout_session_main_id) = $row;

	echo"$get_workout_session_main_id";
}
