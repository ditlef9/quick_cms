...
/**
*
* File: workout_plans/weekly_workout_plan_view_text.php
* Version 1.0.0
...
if($get_current_workout_weekly_id == ""){
	...
	$fh = fopen("$root/_cache/workout_weekly_id_$get_current_workout_weekly_id.txt", "w") or die("can not open file");
...
");
	...
	for($x=0;$x<$title_len;$x++){
		$fh = fopen("$root/_cache/workout_weekly_id_$get_current_workout_weekly_id.txt", "a+") or die("can not open file");
		fwrite($fh, "-");
		fclose($fh);
...
");
	...
	header("Location: $root/_cache/workout_weekly_id_$get_current_workout_weekly_id.txt");
}
