...
if(isset($_SESSION['user_id'])){
	...
	if($process == "1"){
		...
		$result = mysqli_query($link, "UPDATE $t_users SET user_city_name=$inp_city_mysql WHERE user_id=$get_my_user_id");

		if($referer != ""){
			$url = "create_free_account_step_4_dob.php?l=$l&referer=$referer";
		}
		else{
	...
	if($action == ""){
		...
		<!-- Form -->
			<form method=\"POST\" action=\"create_free_account_step_3_city.php?l=$l&amp;process=1"; if($referer != ""){ echo"&amp;referer=$referer"; } echo"\" enctype=\"multipart/form-data\">
			...
			<input type=\"text\" name=\"inp_city\" size=\"25\" value=\"$get_my_user_city_name\" />
		...
		<hr />
		<p>
		<a href=\"create_free_account_step_4_dob.php?user_id=$my_user_id&amp;l=$l"; if($referer != ""){ echo"&amp;referer=$referer"; } echo"\" class=\"btn_default\">$l_skip_this_step</a>
		</p>
		";
	}
}
