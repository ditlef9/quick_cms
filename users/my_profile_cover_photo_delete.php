...
/**
*
* File: users/my_profile_cover_photo_delete.php
...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$l_delete_cover_photo - $l_my_profile - $l_users";
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security']) && $process == "1"){
	...
	// Delete FROM $t_MySQL
	$result = mysqli_query($link, "DELETE FROM $t_users_cover_photos WHERE cover_photo_id='$get_cover_photo_id'");

	// Delete cover photo
	unlink("$root/_uploads/users/images/$get_user_id/cover_photos/$get_cover_photo_destination");

	// Check if this was my set cover photo
	if($get_cover_photo_is_current == "1"){
		// get a new cover photo to use as cover photo
		$query = "SELECT cover_photo_id FROM $t_users_cover_photos WHERE cover_photo_user_id=$user_id_mysql";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		list($get_cover_photo_id) = $row;
		
		if($get_cover_photo_id != ""){
			$result = mysqli_query($link, "UPDATE $t_users_cover_photos SET cover_photo_is_current='1' WHERE cover_photo_id=$get_cover_photo_id");
		}
	}


	// Header
	$url = "my_profile_cover_photo.php?l=$l&ft=success&fm=photo_deleted";
	if($refer != ""){
		$url = $url . "&" . $refer;
	}
