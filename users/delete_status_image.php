...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$l_delete_status_image - $l_users";
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($action == "do_delete_status_image"){
		// Get that status
		...
		if($get_status_id == ""){
			...
			if($get_user_id != "$get_status_user_id"){
				...
				// Update MySQL
				$result = mysqli_query($link, "UPDATE $t_users_status SET status_photo='' WHERE status_id=$status_id_mysql");



				// Header
				$url = "index.php?category=users&page=edit_status&l=$l&ft=success&fm=status_image_deleted";
	...
	elseif($action == ""){
		...
		if($get_status_id == ""){
			...
			if($get_user_id != "$get_status_user_id"){
				...
				echo"
				<h1>$l_delete_status_image</h1>


				<p>
				$l_are_you_sure_you_want_to_delete_status_image
				</p>
				
				<p>";
				$status_security = $get_status_datetime;
				echo"
				<a href=\"index.php?category=users&amp;page=delete_status_image&amp;action=do_delete_status_image&amp;process=1&amp;status_id=$status_id&amp;status_security=$status_security&amp;l=$l\"><img src=\"$root/_webdesign/images/icons/16x16/delete.png\" alt=\"delete.png\" /></a>
				<a href=\"index.php?category=users&amp;page=delete_status_image&amp;action=do_delete_status_image&amp;process=1&amp;status_id=$status_id&amp;status_security=$status_security&amp;l=$l\">$l_delete</a>
				&nbsp;
