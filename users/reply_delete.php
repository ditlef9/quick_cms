...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$l_delete_reply - $l_users";
	...
	if(isset($_SESSION['user_id'])){
		...
		if($get_reply_created_by_user_id == "$get_my_user_id"){
			...
			if($process == "1"){
				...
				$result = mysqli_query($link, "DELETE FROM $t_users_status_replies WHERE reply_id=$get_reply_id") or die(mysqli_error($link));





				$url = "view_profile.php?user_id=$get_reply_user_id&l=$l&ft=success&fm=reply_deletd#status$get_reply_status_id";
				header("Location: $url");
				die;
			...
			<!-- Delete Reply -->
				...
				<p>
				<a href=\"reply_delete.php?reply_id=$reply_id&amp;l=$l&amp;process=1\" class=\"btn_warning\">$l_confirm_delete</a>
				</p>
			<!-- //Delete Reply -->
