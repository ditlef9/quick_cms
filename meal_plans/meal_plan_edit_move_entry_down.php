...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_current_meal_plan_id == ""){
		echo"<p>Meal plan not found.</p>";
			...
			$query = "SELECT entry_id, entry_weight FROM $t_meal_plans_entries WHERE entry_id=$entry_id_mysql AND entry_meal_plan_id=$get_current_meal_plan_id";
			$result = mysqli_query($link, $query);
			$row = mysqli_fetch_row($result);
			list($get_current_entry_id, $get_current_entry_weight) = $row;
			...
			if($get_current_entry_id == ""){
				...
				// Move
				$inp_entry_weight = $get_current_entry_weight+2;
				$result = mysqli_query($link, "UPDATE $t_meal_plans_entries SET entry_weight='$inp_entry_weight' WHERE entry_id='$get_current_entry_id'");
