			...
			if($get_current_entry_food_id != "0"){
				if($process == 1){
					...
					if($inp_entry_food_serving_size == ""){
						$url = "new_meal_plan_step_2_entries.php?meal_plan_id=$meal_plan_id&entry_day_number=$entry_day_number&entry_meal_number=$entry_meal_number&l=$l&action=$action&entry_id=$entry_id";
				...
				if($entry_day_number > 0 && $entry_day_number < 8){
					...
					<!-- Edit form -->
						...
						<form method=\"post\" action=\"new_meal_plan_step_2_entries_edit_entry.php?meal_plan_id=$meal_plan_id&amp;entry_day_number=$entry_day_number&amp;entry_meal_number=$entry_meal_number&amp;l=$l&amp;action=edit_entry&amp;entry_id=$entry_id&amp;process=1\" enctype=\"multipart/form-data\">
				...
				if($process == 1){
					...
					if($inp_entry_serving_size == ""){
						$url = "new_meal_plan_step_2_entries_edit_entry.php?meal_plan_id=$meal_plan_id&entry_day_number=$entry_day_number&entry_meal_number=$entry_meal_number&l=$l&action=$action&entry_id=$entry_id";
						$url = $url . "&ft=error&fm=missing_amount";
						header("Location: $url");
						exit;
					}
