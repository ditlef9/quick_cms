...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_my_member_status == "admin"){
		// Check that I am not the only admin of the team
		// If I am, then I first need to select another admin
		$query = "SELECT count(member_id) FROM $t_rebus_teams_members WHERE member_team_id=$get_current_team_id AND member_status='admin'";
		$result = mysqli_query($link, $query);
		$row = mysqli_fetch_row($result);
		list($get_count_admins) = $row;

		if($get_count_admins == "1"){
			$url = "team_members.php?team_id=$get_current_team_id&ft=info&fm=please_select_another_person_as_admin_before_you_leave_the_team&l=$l";
			header("Location: $url");
			exit;
		}
	}
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$l_leave_team - $get_current_team_name - $l_teams";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
	...
	if($process == "1"){
		...
		mysqli_query($link, "DELETE FROM $t_rebus_teams_members WHERE member_team_id=$get_current_team_id AND member_user_id=$my_user_id_mysql") or die(mysqli_error($link));

		
		$url = "teams.php?ft=success&fm=you_have_left_the_team&l=$l";
		header("Location: $url");
		exit;
	} // process
	...
	<!-- Where am I ? -->
		...
		&gt;
		<a href=\"team_leave.php?team_id=$get_current_team_id&amp;l=$l\">$l_leave_team</a>
		</p>
	<!-- //Where am I ? -->
	...
	<!-- Leave team form-->
		<p>$l_are_you_sure_you_want_to_leave_the_team</p>

		<p>
		<a href=\"team_leave.php?team_id=$get_current_team_id&amp;l=$l&amp;process=1\" class=\"btn_danger\">$l_confirm</a>
		<a href=\"team_open.php?team_id=$get_current_team_id&amp;l=$l\" class=\"btn_default\">$l_cancel</a>
		</p>
