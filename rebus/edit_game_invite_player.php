...
/**
*
* File: rebus/edit_game_invite_player.php
* Version 1.0.0.
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$l_invite_player - $l_invited_players - $get_current_game_title - $l_my_games";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
	...
	if($action == ""){
		if($process == "1"){
			...
			if($inp_email_or_username == ""){
				$url = "edit_game_invite_player.php?game_id=$get_current_game_id&l=$l&ft=error&fm=missing_email_or_username";
				header("Location: $url");
				exit;
			}
			...
			if($get_user_id == ""){
				$url = "edit_game_invite_player.php?game_id=$get_current_game_id&l=$l&ft=error&fm=user_not_found";
				header("Location: $url");
				exit;
			}
			else{
				...
				$query = "SELECT invited_player_id FROM $t_rebus_games_invited_players WHERE invited_player_game_id=$get_current_game_id AND invited_player_user_id=$get_user_id";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
				list($get_invited_player_id) = $row;
				
				if($get_invited_player_id != ""){
					$url = "edit_game_invite_player.php?game_id=$get_current_game_id&l=$l&ft=error&fm=the_user_is_already_invited_of_this_game";
					header("Location: $url");
					exit;
					...
					$inp_title_mysql = quote_smart($link, $get_current_game_title);

					mysqli_query($link, "INSERT INTO $t_rebus_games_invited_players
					(invited_player_id, invited_player_game_id, invited_player_game_title, invited_player_user_id, invited_player_user_name, invited_player_user_email, 
					invited_player_user_photo_destination, invited_player_user_photo_thumb_50, invited_player_invited, invited_player_user_accepted_invitation, invited_player_accepted_by_moderator, 
					invited_player_added_datetime, invited_player_added_date_saying) 
					VALUES 
					(NULL, $get_current_game_id, $inp_title_mysql, $get_user_id, $inp_user_name_mysql, $inp_user_email_mysql,
					$inp_photo_destination_mysql, $inp_photo_thumb_50_mysql, 1, 0, 1,
					'$datetime', '$date_saying')")
					or die(mysqli_error($link));

					$url = "edit_game_invite_player.php?game_id=$get_current_game_id&l=$l&ft=info&fm=user_invited";
					header("Location: $url");
					exit;
				}
			}
		...
		<!-- Where am I ? -->
			...
			&gt;
			<a href=\"edit_game_invite_player.php?game_id=$get_current_game_id&amp;l=$l\">$l_invite_player</a>
			</p>
		<!-- //Where am I ? -->
		...
		<!-- Invite player form -->
			...
			<form method=\"post\" action=\"edit_game_invite_player.php?game_id=$get_current_game_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
			...
			<p><input type=\"submit\" value=\"$l_send_invite\" tabindex=\""; $tabindex=$tabindex+1; echo"$tabindex\" /></p>
			</form>
		<!-- //Invite player form -->
		";
