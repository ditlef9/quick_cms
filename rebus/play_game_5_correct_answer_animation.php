...
/**
*
* File: rebus/play_game_3_correct_answer_animation.php
* Version 1.0.0.
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	$prev_assignment_number = $get_current_session_is_on_assignment_number-1;
	$query = "SELECT assignment_id, assignment_game_id, assignment_number, assignment_type, assignment_value, assignment_address, assignment_answer_a, assignment_answer_a_clean, assignment_answer_b, assignment_answer_b_clean, assignment_radius_metric, assignment_radius_imperial, assignment_radius_user_measurment, assignment_hint_a_value, assignment_hint_a_price, assignment_hint_b_value, assignment_hint_b_price, assignment_hint_c_value, assignment_hint_c_price, assignment_points, assignment_text_when_correct_answer, assignment_time_to_solve_seconds, assignment_time_to_solve_saying, assignment_created_by_user_id, assignment_created_by_ip, assignment_created_datetime, assignment_updated_by_user_id, assignment_updated_by_ip, assignment_updated_datetime FROM $t_rebus_games_assignments WHERE assignment_game_id=$get_current_game_id AND assignment_number=$prev_assignment_number";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	list($get_prev_assignment_id, $get_prev_assignment_game_id, $get_prev_assignment_number, $get_prev_assignment_type, $get_prev_assignment_value, $get_prev_assignment_address, $get_prev_assignment_answer_a, $get_prev_assignment_answer_a_clean, $get_prev_assignment_answer_b, $get_prev_assignment_answer_b_clean, $get_current_assignment_radius_metric, $get_current_assignment_radius_imperial, $get_current_assignment_radius_user_measurment, $get_prev_assignment_hint_a_value, $get_prev_assignment_hint_a_price, $get_prev_assignment_hint_b_value, $get_prev_assignment_hint_b_price, $get_prev_assignment_hint_c_value, $get_prev_assignment_hint_c_price, $get_prev_assignment_points, $get_prev_assignment_text_when_correct_answer, $get_prev_assignment_time_to_solve_seconds, $get_prev_assignment_time_to_solve_saying, $get_prev_assignment_created_by_user_id, $get_prev_assignment_created_by_ip, $get_prev_assignment_created_datetime, $get_prev_assignment_updated_by_user_id, $get_prev_assignment_updated_by_ip, $get_prev_assignment_updated_datetime) = $row;

	if($get_prev_assignment_id == ""){
		echo"Could not find previous assignment";
		die;
	}

	// Find my answer
	$query = "SELECT answer_id, answer_session_id, answer_assignment_id, answer_assignment_number, answer_by_user_group_team, answer_by_user_id, answer_by_group_id, answer_by_team_id, answer_datetime, answer_path, answer_file, answer_text, answer_i_have_flagged_it, answer_is_checked, answer_is_correct, answer_score FROM $t_rebus_games_sessions_answers WHERE answer_session_id=$get_current_session_id AND answer_assignment_id=$get_prev_assignment_id";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	list($get_prev_answer_id, $get_prev_answer_session_id, $get_prev_answer_assignment_id, $get_prev_answer_assignment_number, $get_prev_answer_by_user_group_team, $get_prev_answer_by_user_id, $get_prev_answer_by_group_id, $get_prev_answer_by_team_id, $get_prev_answer_datetime, $get_prev_answer_path, $get_prev_answer_file, $get_prev_answer_text, $get_prev_answer_i_have_flagged_it, $get_prev_answer_is_checked, $get_prev_answer_is_correct, $get_prev_answer_score) = $row;
	if($get_prev_answer_is_correct != "1"){
		echo"Inncorect answer!";
	}

	// Random success sound
	$dir = "_sounds/play_game_3_correct_answer_animation";
	$files = glob($dir . '/*.*');
	...
	$rand = uniqid();
	echo"	<link rel=\"stylesheet\" type=\"text/css\" href=\"_css/play_game_5_correct_answer_animation.css?r=$rand\" />\n";
	...
	echo"	<meta http-equiv=\"refresh\" content=\"4;url=play_game_3_assignments.php?game_id=$get_current_game_id&amp;session_id=$get_current_session_id&amp;l=$l\" />\n";
	echo"</head>\n";
	echo"<body>\n";
	echo"<div class=\"background\""; 
	if(file_exists("../$get_prev_answer_path/$get_prev_answer_file") && $get_prev_answer_file != ""){
		echo" style=\"background: url('../$get_prev_answer_path/$get_prev_answer_file');background-size: cover;\"";
	}
	else{
	...
	<!-- Center -->
		<div class=\"center\">
			...
			<!-- //Random font -->
			<h1 id=\"fontfamily\">$get_prev_assignment_text_when_correct_answer</h1>
