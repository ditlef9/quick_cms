...
// Liquidbase
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_admin_liquidbase(
	...
	echo("MySQLI error: " . $mysqli -> error); die;
}


if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_profile_photo(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_status(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_status_likes(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_friends(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_friends_requests(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS  $t_users_cover_photos(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_email_subscriptions(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_antispam_questions(
...
$result = $mysqli->query("SELECT * FROM $t_users_antispam_questions LIMIT 1");
if($result->num_rows == 0){
	if (!$mysqli -> query("INSERT INTO $t_users_antispam_questions
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_antispam_answers(
...
$result = $mysqli->query("SELECT * FROM $t_users_antispam_answers LIMIT 1");
if($result->num_rows == 0){
	if (!$mysqli -> query("INSERT INTO $t_users_antispam_answers
...
// Users :: API Sessions
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_users_api_sessions(
...
// Pages
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_pages(
...
// Images 
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_images(
...
// Images :: Paths
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_images_paths(
...
// Images :: Paths
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_site_translations_directories(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_site_translations_files(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_site_translations_strings(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_admin_translations_directories(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_admin_translations_files(
...
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_admin_translations_strings(
...
// Banned: Hostnames
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_banned_hostnames(
...
$result = $mysqli->query("SELECT * FROM $t_banned_hostnames LIMIT 1");
if($result->num_rows == 0){
	...
	for($x=0;$x<sizeof($array);$x++){
		...
		if($result->num_rows == 0){
			// Insert nanned hostname
			$stmt = $mysqli->prepare("INSERT INTO $t_banned_hostnames
			(banned_hostname_id, banned_hostname, banned_hostname_datetime, banned_hostname_reason) 
			VALUES 
			(NULL, ?, ?, ?)");
			$stmt->bind_param("sss", $content, $datetime, $reason); 
			$stmt->execute();
		}
...
// Banned: IPS
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_banned_ips(
...
$result = $mysqli->query("SELECT * FROM $t_banned_ips LIMIT 1");
if($result->num_rows == 0){
	...
	for($x=0;$x<sizeof($array);$x++){
		...
		$stmt = $mysqli->prepare("SELECT banned_ip FROM $t_banned_ips WHERE banned_ip=?");
		$stmt->bind_param("s", $content);
		$stmt->execute();
		$result = $stmt->get_result();
		if($result->num_rows == 0){
			// Insert banned ip
			$stmt = $mysqli->prepare("INSERT INTO $t_banned_ips
...
// Banned: User Agents
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_banned_user_agents(
...
$result = $mysqli->query("SELECT * FROM $t_banned_user_agents LIMIT 1");
if($result->num_rows == 0){
	...
	for($x=0;$x<sizeof($array);$x++){
		...
		$result = $stmt->get_result();
		if($result->num_rows == 0){
			// Insert banned user_agent
			// Insert banned ip
			$stmt = $mysqli->prepare("INSERT INTO $t_banned_user_agents
...
// Analytics
if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_analytics(
