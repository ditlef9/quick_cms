...
$query = "SELECT recipe_favorite_id FROM $t_recipes_favorites WHERE recipe_favorite_recipe_id=$get_recipe_id AND recipe_favorite_user_id=$get_user_id";
$result = mysqli_query($link, $query);
$row = mysqli_fetch_row($result);
list($get_recipe_favorite_id) = $row;
if($get_recipe_favorite_id != ""){
	echo"Already your favorite";
}
else{
	$inp_date = date('j M Y');
	...
	VALUES 
	(NULL, $get_recipe_id, $get_user_id, '$inp_date')")
	or die(mysqli_error($link));
	
	echo"Favorite added for $get_my_user_alias at $inp_date";
}
