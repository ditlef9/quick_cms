...
if($get_occasion_id == ""){
	...
	$website_title = "$get_occasion_translation_value - $l_occasions - $l_recipes";
}
...
/*- Content ---------------------------------------------------------------------------------- */
	...
	<!-- Adapter view -->";
		...
		<b>$l_show_per:</b>
		<input type=\"checkbox\" name=\"inp_show_hundred_metric\" class=\"onclick_go_to_url\" data-target=\"user_adapted_view.php?set=hundred_metric&amp;process=1&amp;referer=occasions_browse&amp;occasion_id=$occasion_id&amp;l=$l\""; if($get_current_view_hundred_metric == "1"){ echo" checked=\"checked\""; } echo" /> $l_hundred
		<input type=\"checkbox\" name=\"inp_show_pcs_metric\" class=\"onclick_go_to_url\" data-target=\"user_adapted_view.php?set=serving&amp;process=1&amp;referer=occasions_browse&amp;occasion_id=$occasion_id&amp;l=$l\""; if($get_current_view_serving == "1"){ echo" checked=\"checked\""; } echo" /> $l_serving
		<input type=\"checkbox\" name=\"inp_show_metric_us_and_or_pcs\" class=\"onclick_go_to_url\" data-target=\"user_adapted_view.php?set=eight_us&amp;process=1&amp;referer=occasions_browse&amp;occasion_id=$occasion_id&amp;l=$l\""; if($get_current_view_eight_us == "1"){ echo" checked=\"checked\""; } echo" /> $l_eight
		</p>
	...
	$x = 0;
	$count_recipes = 0;
	$query = "SELECT $t_recipes.recipe_id, $t_recipes.recipe_title, $t_recipes.recipe_introduction, $t_recipes.recipe_image_path, $t_recipes.recipe_image_h_a, $t_recipes.recipe_thumb_h_a_278x156, $t_recipes.recipe_unique_hits FROM $t_recipes JOIN $t_recipes_numbers ON $t_recipes.recipe_id=$t_recipes_numbers.number_recipe_id WHERE $t_recipes.recipe_occasion_id='$get_occasion_id' AND $t_recipes.recipe_language=$l_mysql AND $t_recipes.recipe_published=1";
	// Order
	...
	while($row = mysqli_fetch_row($result)) {
		...
		if($get_recipe_image_h_a != ""){
			...
			if($get_occasion_last_updated != "$date"){
				$inp_occasion_image = "$get_recipe_image_path/$get_recipe_image_h_a";
