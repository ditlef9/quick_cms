...
/**
*
* File: food/new_new_workout_plan_yearly_step_4_image.php
* Version 1.0.0
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	if($get_workout_yearly_id == ""){
			...
			if($file_type == "jpg" OR $file_type == "jpeg" OR $file_type == "png" OR $file_type == "gif"){
				...
				if(move_uploaded_file($_FILES['inp_image']['tmp_name'], $target_path)) {
					...
					if(is_numeric($width) && is_numeric($height)){
						...
						if($width < 949){
							unlink($target_path);
							$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=width_have_to_be_bigger&width=$width&height=$height";
							header("Location: $url");
							exit;
						}
						if($height < 639){
							unlink($target_path);
							$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=height_have_to_be_bigger&width=$width&height=$height";
							header("Location: $url");
							exit;
						}
						...
						// Header
						$url = "yearly_workout_plan_view.php?yearly_id=$yearly_id&l=$l&ft=success&fm=image_uploaded";
						header("Location: $url");
						exit;
					}
					else{
						...
						unlink("$target_path");
						$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=file_is_not_an_image";
						header("Location: $url");
						exit;
					}
				}
					...
					case 1:
						$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$ll&ft=error&fm=to_big_file";
						header("Location: $url");
						exit;
						break;
					case 2:
						$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=to_big_file";
						header("Location: $url");
						exit;
						break;
					case 3:
						$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=only_parts_uploaded";
						header("Location: $url");
						exit;
						break;
					case 4:
						$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=no_file_uploaded";
						header("Location: $url");
						exit;
						break;
					}
				...
				$url = "new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&l=$l&ft=error&fm=invalid_file_type&file_type=$file_type";
				header("Location: $url");
				exit;
			} // file type end
		...
		<!-- Form -->
			...
			<form method=\"post\" action=\"new_workout_plan_yearly_step_4_image.php?yearly_id=$yearly_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
