...
/**
*
* File: workout_plans/weekly_workout_plan_delete_comment.php
* Version 1.0.0
...
if($get_current_comment_id == ""){
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		if($get_current_comment_user_id == "$my_user_id" OR $get_my_user_rank == "admin" OR $get_my_user_rank == "moderator"){
			...
			/*- Headers ---------------------------------------------------------------------------------- */
			$website_title = "$l_delete_comment - $get_current_workout_weekly_title - $l_workout_plans";
			include("$root/_webdesign/header.php");


			/*- Content ---------------------------------------------------------------------------------- */
			if($process == "1"){
				mysqli_query($link, "DELETE FROM $t_workout_plans_weekly_comments WHERE comment_id=$get_current_comment_id") or die(mysqli_error($link));


				// Header
				$url = "weekly_workout_plan_view.php?weekly_id=$get_current_workout_weekly_id&l=$l#comments";
				header("Location: $url");
				exit;
			}
			echo"
			...
			&gt;
			<a href=\"weekly_workout_plan_delete_comment.php?comment_id=$get_current_comment_id&amp;l=$l\">$l_delete_comment</a>
			</p>
			<!-- //Where am I ? -->
			...
			<p>
			<a href=\"weekly_workout_plan_delete_comment.php?comment_id=$get_current_comment_id&amp;l=$l&amp;process=1\" class=\"btn_default\">$l_confirm</a>
			</p>
