...
/**
*
* File: _admin/_inc/discuss/form_edit_questions_edit.php
* Version 1
...
if($get_current_form_id == ""){
	...
	$query = "SELECT form_question_id, form_id, form_question, form_question_type, form_question_options, form_question_help_text FROM $t_forum_forms_questions WHERE form_question_id=$question_id_mysql";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	list($get_current_form_question_id, $get_current_form_id, $get_current_form_question, $get_current_form_question_type, $get_current_form_question_options, $get_current_form_question_help_text) = $row;
	...
	if($get_current_form_question_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "UPDATE $t_forum_forms_questions SET 
					 form_question=$inp_question_mysql, 
					 form_question_type=$inp_type_mysql, 
					 form_question_options=$inp_options_mysql,
					form_question_help_text=$inp_help_text_mysql
			 WHERE form_question_id=$get_current_form_question_id");



			$url = "index.php?open=$open&page=form_edit_questions&form_id=$get_current_form_id&editor_language=$editor_language&ft=success&fm=changes_saved";
			header("Location: $url");
			exit;
		}
		echo"
		...
		<form method=\"post\" action=\"index.php?open=$open&amp;page=$page&amp;form_id=$form_id&amp;question_id=$question_id&amp;editor_language=$editor_language&amp;process=1\" enctype=\"multipart/form-data\">

		<p><b>Question:</b><br />
		<input type=\"text\" name=\"inp_question\" value=\"$get_current_form_question\" size=\"40\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\" />
		</p>

		<p><b>Type:</b><br />
		<select name=\"inp_type\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\">
			<option value=\"text\""; if($get_current_form_question_type == "text"){ echo" selected=\"selected\""; } echo">Text</option>
			<option value=\"textarea\""; if($get_current_form_question_type == "textarea"){ echo" selected=\"selected\""; } echo">Textarea</option>
			<option value=\"select\""; if($get_current_form_question_type == "select"){ echo" selected=\"selected\""; } echo">Select</option>
		</select>

		<p><b>Select options:</b><br />
		<textarea name=\"inp_options\" rows=\"10\" cols=\"60\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\">$get_current_form_question_options</textarea>
		</p>

		<p><b>Help text:</b><br />
		<input type=\"text\" name=\"inp_help_text\" value=\"$get_current_form_question_help_text\" size=\"40\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\" />
		</p>
