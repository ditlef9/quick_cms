...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$l_team_members - $get_current_team_name - $l_teams";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
	...
	if($action == ""){
		...
		<!-- Where am I ? -->
			...
			&gt;
			<a href=\"team_members.php?team_id=$get_current_team_id&amp;l=$l\">$l_team_members</a>
			</p>
		<!-- //Where am I ? -->
		...
		<!-- Actions -->
			<p>
			<a href=\"team_members.php?team_id=$get_current_team_id&amp;action=invite_member&amp;l=$l\" class=\"btn_default\">$l_invite_member</a>
			</p>
		<!-- //Actions -->
		...
		<!-- Team members -->
			<table class=\"hor-zebra\">
			 ...
			 <tbody>";
			$query = "SELECT member_id, member_team_id, member_user_id, member_user_name, member_user_email, member_user_photo_destination, member_user_photo_thumb_50, member_status, member_invited, member_user_accepted_invitation, member_accepted_by_moderator, member_joined_datetime, member_joined_date_saying FROM $t_rebus_teams_members WHERE member_team_id=$get_current_team_id ORDER BY member_user_name ASC";
			$result = mysqli_query($link, $query);
			while($row = mysqli_fetch_row($result)) {
				list($get_member_id, $get_member_team_id, $get_member_user_id, $get_member_user_name, $get_member_user_email, $get_member_user_photo_destination, $get_member_user_photo_thumb_50, $get_member_status, $get_member_invited, $get_member_user_accepted_invitation, $get_member_accepted_by_moderator, $get_member_joined_datetime, $get_member_joined_date_saying) = $row;
				// Style
				$style = "";
				 ...
				 <tr>
				  <td"; if($style != ""){ echo" class=\"$style\""; } echo">
					...
					if($get_my_member_status == "admin" OR $get_my_member_status == "moderator"){
						echo"<a href=\"team_members.php?action=view_member&amp;team_id=$get_current_team_id&amp;member_id=$get_member_id&amp;l=$l\">$get_member_user_name</a>";
					}
					else{
				  ...
				  <td"; if($style != ""){ echo" class=\"$style\""; } echo">
					...
					if($get_member_accepted_by_moderator == "0"){
						...
						if($get_my_member_status == "admin" OR $get_my_member_status == "moderator"){
							echo"<a href=\"team_members.php?action=verify_member&amp;team_id=$get_current_team_id&amp;member_id=$get_member_id&amp;l=$l&amp;process=1\" class=\"btn_default\">$l_verify_user</a>";
						}
					}
					echo"</span>
				  </td>
		...
		if($get_my_member_status == "admin" OR $get_my_member_status == "moderator"){
			if($process == "1"){
				...
				if($inp_username_or_email == ""){
					$url = "team_members.php?group_id=$get_current_group_id&action=invite_member&l=$l&ft=error&fm=missing_username_or_email";
					header("Location: $url");
					exit;
				}
				...
				if($get_user_id == ""){
					...
					if(strpos($inp_username_or_email, "@") !== false){
						...
						$message = $message . "$l_join_the_team_by_following_the_link_below:<br />\n";
						$message = $message . "<a href=\"$configSiteURLSav/rebus/team_accept_invitation.php?team_id=$get_team_id&amp;l=$l\">$l_accept</a>";
						$message = $message . "</a>";
						...
						VALUES 
						(NULL, $get_current_team_id, $get_user_id, $inp_username_mysql, $inp_email_mysql,
						'', '', $inp_status_mysql,  1, 0, 
						1, '$datetime', '$date_saying')")
						or die(mysqli_error($link));


						$url = "team_members.php?team_id=$get_current_team_id&action=invite_member&l=$l&ft=info&fm=user_invited";
						header("Location: $url");
						exit;
						...
						// @ not found
						$url = "team_members.php?team_id=$get_current_team_id&action=invite_member&l=$l&ft=error&fm=user_not_found";
						header("Location: $url");
						exit;
					}
				}
					...
					$query = "SELECT member_id FROM $t_rebus_teams_members WHERE member_team_id=$get_current_team_id AND member_user_id=$get_user_id";
					$result = mysqli_query($link, $query);
					$row = mysqli_fetch_row($result);
					list($get_member_id) = $row;
					if($get_member_id != ""){
						$url = "team_members.php?team_id=$get_current_team_id&action=invite_member&l=$l&ft=error&fm=user_already_member_of_team";
						header("Location: $url");
						exit;
					}
					else{
						...
						member_accepted_by_moderator, member_joined_datetime, member_joined_date_saying) 
						VALUES 
						(NULL, $get_current_team_id, $get_user_id, $inp_user_name_mysql, $inp_user_email_mysql, 
						$inp_photo_destination_mysql, $inp_photo_thumb_50_mysql, $inp_status_mysql, 1, 0, 
						1, '$datetime', '$date_saying')")
						or die(mysqli_error($link));
			...
			<!-- Where am I ? -->
				...
				<a href=\"team_members.php?team_id=$get_current_team_id&amp;l=$l\">$l_team_members</a>
				&gt;
				<a href=\"team_members.php?team_id=$get_current_team_id&amp;action=invite_member&amp;l=$l\">$l_invite_member</a>
				</p>
			<!-- //Where am I ? -->
			...
			<!-- Invite member form -->
				...
				<p>
				$l_here_you_can_invite_family_friends_and_colleagues_to_your_team.
				$l_invited_people_will_get_an_email_with_a_link_that_they_have_to_click_on_to_accept_the_invite.
				</p>


			<form method=\"post\" action=\"team_members.php?team_id=$get_current_team_id&amp;action=invite_member&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
		...
		if($get_my_member_status == "admin" OR $get_my_member_status == "moderator"){
			...
			if($get_current_member_id == ""){
				...
				if($process == "1"){
					...
					if($inp_status != "member" && $inp_status != "moderator" && $inp_status != "admin"){
						if($inp_status == "admin" && $get_my_member_status == "admin"){
							...
							$url = "team_members.php?team_id=$get_current_team_id&action=$action&member_id=$get_current_member_id&l=$l&ft=error&fm=unknown_status";
							header("Location: $url");
							exit;
						}
					}
					...
					$url = "team_members.php?team_id=$get_current_team_id&action=$action&member_id=$get_current_member_id&l=$l&ft=info&fm=changes_saved";
					header("Location: $url");
					exit;
				...
				<!-- Where am I ? -->
					...
					<a href=\"team_open.php?team_id=$get_current_team_id&amp;l=$l\">$get_current_team_name</a>
					&gt;
					<a href=\"team_open.php?team_id=$get_current_team_id&amp;l=$l\">$l_team_members</a>
					&gt;
					<a href=\"team_open.php?team_id=$get_current_team_id&amp;action=$action&amp;member_id=$get_current_member_id&amp;l=$l\">$get_current_member_user_name</a>
					</p>
				<!-- //Where am I ? -->
				...
				<!-- Edit member form -->
				<form method=\"post\" action=\"team_members.php?team_id=$get_current_team_id&amp;action=$action&amp;member_id=$get_current_member_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
				...
				<a href=\"team_members.php?team_id=$get_current_team_id&amp;action=delete_member&amp;member_id=$get_current_member_id&amp;l=$l\" class=\"btn_warning\">$l_delete</a>
				</p>
		...
		if($get_my_member_status == "admin" OR $get_my_member_status == "moderator"){
			...
			if($get_current_member_id == ""){
				...
				if($process == "1"){
					...
					mysqli_query($link, "DELETE FROM $t_rebus_teams_members 
						WHERE member_id=$get_current_member_id") or die(mysqli_error($link));


					$url = "team_members.php?team_id=$get_current_team_id&l=$l&ft=info&fm=member_deleted";
					header("Location: $url");
					exit;
				...
				<!-- Where am I ? -->
					...
					<a href=\"team_open.php?team_id=$get_current_team_id&amp;l=$l\">$l_team_members</a>
					&gt;
					<a href=\"team_open.php?team_id=$get_current_team_id&amp;action=view_member&amp;member_id=$get_current_member_id&amp;l=$l\">$get_current_member_user_name</a>
					&gt;
					<a href=\"team_open.php?team_id=$get_current_team_id&amp;action=$action&amp;member_id=$get_current_member_id&amp;l=$l\">$l_delete</a>
					</p>
				<!-- //Where am I ? -->
				...
				<!-- Delete member form -->
					...
					<p>
					<a href=\"team_members.php?team_id=$get_current_team_id&amp;action=$action&amp;member_id=$get_current_member_id&amp;l=$l&amp;process=1\" class=\"btn_danger\">$l_confirm</a>
					</p>
