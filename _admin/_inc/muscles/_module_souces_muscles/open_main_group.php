...
/**
*
* File: _admin/_inc/muscles/open_main_group.php
* Version 1.0.0
...
if($get_current_main_muscle_group_id == ""){
	...
	$query_translation = "SELECT muscle_group_translation_id,muscle_group_translation_name, muscle_group_translation_text FROM $t_muscle_groups_translations WHERE muscle_group_translation_muscle_group_id=$get_current_main_muscle_group_id AND muscle_group_translation_language=$l_mysql";
	$result_translation = mysqli_query($link, $query_translation);
	$row_translation = mysqli_fetch_row($result_translation);
	list($get_current_main_muscle_group_translation_id, $get_current_main_muscle_group_translation_name, $get_current_main_muscle_group_translation_text) = $row_translation;
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$get_current_main_muscle_group_translation_name - $l_muscles";
	include("$root/_webdesign/header.php");
	...
	<!-- Muscles in main group -->
		 ...
		 <thead>
		  <tr>
		   <th scope=\"col\" colspan=\"2\">
			<a href=\"$root/muscles/open_main_group.php?main_group_id=$get_current_main_muscle_group_id&amp;l=$l\" style=\"font-weight: bold;\">$get_current_main_muscle_group_translation_name</a>
		   </td>
		  </tr>
		 </thead>
		...
		";
		// Get sub
		$query_sub = "SELECT muscle_group_id, muscle_group_name, muscle_group_name_clean, muscle_group_parent_id, muscle_group_image_path, muscle_group_image_file FROM $t_muscle_groups WHERE muscle_group_parent_id='$get_current_main_muscle_group_id'";
		...
		while($row_sub = mysqli_fetch_row($result_sub)) {
			...
			echo"
			 <tr>
			  <td class=\"$style\">
				<a href=\"$root/muscles/open_sub_group.php?main_group_id=$get_current_main_muscle_group_id&amp;sub_group_id=$get_sub_muscle_group_id&amp;l=$l\">$get_sub_muscle_group_translation_name</a>
			  </td>
			  <td class=\"$style\">
				<span>";
				...
				while($row_m = mysqli_fetch_row($result_m)) {
					...
					echo"
					<a href=\"$root/muscles/muscle.php?main_group_id=$get_current_main_muscle_group_id&amp;sub_group_id=$get_sub_muscle_group_id&amp;muscle_id=$get_muscle_id&amp;l=$l\">$get_muscle_translation_simple_name</a>
				...
				while($row_p = mysqli_fetch_row($result_p)) {
					...
					echo"
					<a href=\"$root/muscles/open_muscle_part_of.php?main_group_id=$get_current_main_muscle_group_id&amp;sub_group_id=$get_sub_muscle_group_id&amp;part_of_id=$get_muscle_part_of_id&amp;l=$l\">$get_muscle_part_of_translation_name</a>
					(";
					...
					while($row_m = mysqli_fetch_row($result_m)) {
						...
						echo"
						<a href=\"$root/muscles/muscle.php?main_group_id=$get_current_main_muscle_group_id&amp;sub_group_id=$get_sub_muscle_group_id&amp;part_of_id=$get_muscle_part_of_id&amp;muscle_id=$get_muscle_id&amp;l=$l\">";
	...
	<!-- Main Image -->
		";
		if($get_current_main_muscle_group_image_file != "" && file_exists("$root/$get_current_main_muscle_group_image_path/$get_current_main_muscle_group_image_file")){
			echo"<p><img src=\"$root/$get_current_main_muscle_group_image_path/$get_current_main_muscle_group_image_file\" alt=\"$get_current_main_muscle_group_image_file\" /></p>";
		}
		echo"
	<!-- //Main Image -->
