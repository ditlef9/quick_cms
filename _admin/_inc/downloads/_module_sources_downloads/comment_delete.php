...
/**
*
* File: downloads/comment_delete.php
* Version 1.0.0
...
if($get_current_comment_id == ""){
	...
	if($get_current_download_id == ""){
		...
		/*- Headers ---------------------------------------------------------------------------------- */
		$website_title = "$l_delete_comment - $get_current_download_title - $get_current_main_category_title - $l_downloads";
		include("$root/_webdesign/header.php");
		...
		if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
			...
			if($can_edit == "0"){
				...
				if($process == "1"){
					// Update
					mysqli_query($link, "DELETE FROM $t_downloads_comments WHERE comment_id=$get_current_comment_id") or die(mysqli_error($link));

					// header
					$url = "view_download.php?download_id=$get_current_download_id&l=$l&ft_comment=success&fm_comment=comment_deleted#comments";
					header("Location: $url");
					exit;
				}
