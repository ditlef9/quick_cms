...
if(isset($_POST['inp_user_height'])){
	$inp_user_height = $_POST['inp_user_height'];
	$inp_user_height = output_html($inp_user_height);
	$inp_user_height_mysql = quote_smart($link, $inp_user_height);
}
else{
	$fm = "Missing height";
}
...
$result = mysqli_query($link, "UPDATE $t_users SET user_height=$inp_user_height_mysql, user_last_online=$inp_user_last_online_mysql, user_last_ip=$inp_user_last_ip_mysql, user_synchronized=$inp_user_synchronized_mysql WHERE user_id=$get_user_id");

echo"Height updated ok";
