...
while($row_d = mysqli_fetch_row($result_d)) {
	...
	if($get_current_domain_id == ""){
		...
		mysqli_query($link, "UPDATE $t_domains_monitoring_domains_index SET domain_checked_ip_by_script=1 WHERE domain_id=$get_current_domain_id") or die(mysql_error($link));
		...
		$query_k = "SELECT keyword_id, keyword_group_id, keyword_user_id, keyword_title, keyword_type, keyword_value, keyword_value_length, keyword_combinations, keyword_domain_tlds FROM $t_domains_monitoring_filters_keywords WHERE keyword_type='ip_exact' OR keyword_type='host_exact'";
			...
			*/
			...
			if($check_this_keyword == "1"){
				...
				if($get_current_domain_ip == ""){
					// Find IP
					$get_current_domain_ip = gethostbyname($get_current_domain_value); 
					if($get_current_domain_ip == ""){
						$get_current_domain_ip = "NA";
					}
					elseif($get_current_domain_ip == "$get_current_domain_value"){
						$get_current_domain_ip = gethostbyname(parse_url($get_current_domain_value, PHP_URL_HOST));
					}
					$get_current_domain_ip = output_html($get_current_domain_ip);
			
					// Host addr
					$get_current_domain_host_addr = "NA";
					if (filter_var($get_current_domain_ip, FILTER_VALIDATE_IP)) {
						// echo"<p><b>Domain IP:</b> $get_current_domain_ip</p>\n";
						$get_current_domain_host_addr = gethostbyaddr($get_current_domain_ip);
						if($get_current_domain_host_addr == "" OR $get_current_domain_host_addr == "$get_current_domain_ip"){
							$get_current_domain_host_addr = "NA";
						}
						$get_current_domain_host_addr = output_html($get_current_domain_host_addr);
					}

					// Host name and URL
					$get_current_domain_host_name = "NA";
					$get_current_domain_host_url = "NA";
					if($get_current_domain_host_addr != "NA"){
						$get_current_domain_host_url = get_domain($get_current_domain_host_addr);
						$get_current_domain_host_name = strtolower($get_current_domain_host_url);
						$get_current_domain_host_url = "https://$get_current_domain_host_url";
					}
					$get_current_domain_host_name = output_html($get_current_domain_host_name);
					$get_current_domain_host_url = output_html($get_current_domain_host_url);
					...
					<table>
					 <tr>
					  ...
					  <td>
						<span>$get_current_domain_value</span>
					  </td>
					 </tr>
					";
					echo"
					 <tr>
					  ...
					  <td>
						<span>$get_current_domain_ip</span>
					  </td>
					 </tr>
					 <tr>
					  <td>
					  ...
					  <td>
						<span>$get_current_domain_host_addr</span>
					  </td>
					 </tr>
					 <tr>
					  <td>
						<span>Host name and URL:</span>
					  </td>
					  <td>
						<span>$get_current_domain_host_name</span>
					  </td>
					 </tr>
					</table>
				...
				if($get_check_keyword_type == "ip_exact"){
					// Check IP
					if($get_current_domain_ip == "$get_check_keyword_value"){
						$check_result = "True";
						$inp_filters_activated = "IP exact $get_check_keyword_value ($get_check_keyword_title)";
						$style = "success";
						$inp_score = 15;
					}
				}
				elseif($get_check_keyword_type == "host_exact"){
					// Check host
					if($get_current_domain_host_name == "$get_check_keyword_value"){
						$check_result = "True";
						$inp_filters_activated = "Host exact $get_check_keyword_value ($get_check_keyword_title)";
						$style = "success";
						$inp_score = 15;
					}
				}
				...
				if($check_result == "True"){
					// Echo results
					echo"<p style=\"color: green;\">$inp_filters_activated</p>\n";
	
					// Domain checked (Update domain)
					mysqli_query($link, "UPDATE $t_domains_monitoring_domains_index SET 
							domain_filters_activated=1, 
							domain_checked_ip_by_script=1, 
							domain_checked_other_by_script=1, 
							domain_checked_starts_with_ends_with_by_script=1 
							WHERE domain_id=$get_current_domain_id") or die(mysql_error($link));
...
if($get_current_domain_id != ""){
	echo"
	<meta http-equiv=\"refresh\" content=\"0;url=check_domains_script.php?inc=2_check_regex_contains_starts_with_ends_with&amp;domain_id=$get_current_domain_id&amp;start_time=$start_time\" />
	";
}
else{
	echo"
