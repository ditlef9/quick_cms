...
/**
*
* File: references/_includes/reference_by_alphabet.php
* Version 2.0.0
...
if($get_current_reference_id != ""){
	...
	if($mode == ""){
		...
		// Headline
		echo"
		<h1><a href=\"index.php?l=$l\"><img src=\"_gfx/$get_current_reference_icon_96\" alt=\"$get_current_reference_icon_96\" style=\"float: right;padding: 0px 0px 10px 10px;\" /></a>$get_current_reference_title $l_by_alphabet_headline";
		...
		<!-- Reference by alphabet -->
			...
			// Get guides
			$query_lessons = "SELECT guide_id, guide_title, guide_title_clean, guide_title_short, guide_title_length, guide_short_description, guide_group_id FROM $t_references_index_guides WHERE guide_reference_id=$get_current_reference_id";
			if(isset($search_query)){
				// Ready for MySQL search
				$search_query = $search_query . "%";
				$search_query_mysql = quote_smart($link, $search_query);
				$query_lessons = $query_lessons . " AND guide_title LIKE $search_query_mysql";
			}
			else{
				$query_lessons = $query_lessons . " ORDER BY guide_title ASC";
			}
			$result_lessons = mysqli_query($link, $query_lessons);
			while($row_lessons = mysqli_fetch_row($result_lessons)) {
				list($get_guide_id, $get_guide_title, $get_guide_title_clean, $get_guide_title_short, $get_guide_title_length, $get_guide_short_description, $get_guide_group_id) = $row_lessons;
