...
<!-- ads_index -->
	";
	if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_ads_index(
	...
	$query = "SELECT count(ad_id) FROM $t_ads_index";
	$result = $mysqli->query($query);
	$row = $result->fetch_row();
	list($sql_count_ad_id) = $row;
	if ($sql_count_ad_id == 0){



		$result = $mysqli->query("INSERT INTO $t_ads_index
	...
	echo"
	<p>$t_ads_index: $sql_count_ad_id</p>
<!-- ads_index -->


<!-- advertisers -->
	";
	if (!$mysqli -> query("CREATE TABLE IF NOT EXISTS $t_ads_advertisers(
	...
	$query = "SELECT count(advertiser_id) FROM $t_ads_advertisers";
	$result = $mysqli->query($query);
	$row = $result->fetch_row();
	list($sql_count_advertiser_id) = $row;

	if ($sql_count_advertiser_id == 0){
		$result = $mysqli->query("INSERT INTO $t_ads_advertisers
	...
	echo"
	<p>$t_ads_advertisers: $sql_count_advertiser_id</p>
<!-- advertisers -->
";
