...
if($get_current_blog_info_id == ""){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$get_current_blog_title - $l_blog";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
	...
	<!-- Blog navigation -->
		<div class=\"tabs\">
			<ul>
				...
				<li><a href=\"view_blog_links.php?info_id=$get_current_blog_info_id&amp;l=$l\">$l_links</a></li>
			</ul>
		</div>
		<div class=\"clear\"></div>
	<!-- //Blog navigation -->
	...
	<!-- Show last posts in this blog -->
		...
		while($row_w = mysqli_fetch_row($result_w)) {
			...
			if($show_post == "true" && $get_blog_post_image_file != "" && file_exists("$root/$get_blog_post_image_path/$get_blog_post_image_file")){
				...
				if($get_blog_post_category_id != "0" && $get_blog_post_category_id != ""){
