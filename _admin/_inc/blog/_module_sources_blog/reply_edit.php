...
if($get_current_reply_id == ""){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$l_edit_reply - $get_current_blog_post_title - $get_current_blog_title - $l_blog";
	if(file_exists("./favicon.ico")){ $root = "."; }
	elseif(file_exists("../favicon.ico")){ $root = ".."; }
	elseif(file_exists("../../favicon.ico")){ $root = "../.."; }
	elseif(file_exists("../../../favicon.ico")){ $root = "../../.."; }
		...
		if($can_edit == "0"){
			...
			if($process == "1"){
				...
				mysqli_query($link, "UPDATE $t_blog_posts_comments_replies SET 
							reply_text=$inp_text_mysql,
							reply_updated='$datetime',
							reply_updated_saying='$date_saying'
							WHERE reply_id=$get_current_reply_id")
							or die(mysqli_error($link));

				// header
				$url = "view_post.php?post_id=$get_current_blog_post_id&amp;l=$l&ft_comment=success&fm_comment=changes_saved#reply$get_current_reply_id";
				header("Location: $url");
				exit;
			}
			...
			<!-- Where am I? -->
				...
				&gt;
				<a href=\"reply_edit.php?reply_id=$get_current_reply_id&amp;l=$l\">$l_edit_reply $get_current_reply_id</a>
				</p>
			<!-- //Where am I? -->
	
			<!-- Edit comment form -->

				<form method=\"post\" action=\"reply_edit.php?reply_id=$get_current_reply_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
				...
				<table>
				  ...
				  <td style=\"vertical-align: top;padding-right: 10px;text-align:center;\">
					<p>
					";
					if(file_exists("$root/$get_current_reply_by_user_image_path/$get_current_reply_by_user_image_thumb_60") && $get_current_reply_by_user_image_thumb_60 != ""){

				
						echo"
						<img src=\"$root/$get_current_reply_by_user_image_path/$get_current_reply_by_user_image_thumb_60\" alt=\"$get_current_reply_by_user_image_thumb_60\" />
						<br />
						";
					}
					echo"
					$get_current_reply_by_user_name
					</p>
				  </td>
				  <td style=\"vertical-align: top;\">
					<p>
					<textarea name=\"inp_text\" rows=\"5\" cols=\"80\">";
					$get_current_reply_text = str_replace("<br />", "\n", $get_current_reply_text);
					echo"$get_current_reply_text</textarea><br />
					<input type=\"submit\" value=\"$l_save_reply\" class=\"btn_default\" />
					</p>
				  </td>
				 </tr>
				</table>
