...
if($get_current_comment_id == ""){
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		if($get_likes_dislike_id == ""){
			...
			VALUES 
			(NULL, $get_current_comment_blog_post_id, $get_current_comment_blog_info_id, $get_current_comment_id, $get_my_user_id, 'dislike')")
			or die(mysqli_error($link));

			// Update blog post
			$inp_comment_dislikes = $get_current_comment_dislikes+1;
			mysqli_query($link, "UPDATE $t_blog_posts_comments SET comment_dislikes=$inp_comment_dislikes WHERE comment_id=$get_current_comment_id") or die(mysqli_error($link));

			// Header
			$url = "view_post.php?post_id=$get_current_comment_blog_post_id&l=$l&ft_comment=success&fm_comment=comment_disliked#comment$get_current_comment_id";
			header("Location: $url");
			exit;
		}
		else{
			...
			if($get_likes_dislike_direction == "like"){
				...
				mysqli_query($link, "UPDATE $t_blog_posts_comments_likes_dislikes SET likes_dislike_direction='dislike' WHERE likes_dislike_id=$get_likes_dislike_id") or die(mysqli_error($link));

				// Header
				$url = "view_post.php?post_id=$get_current_comment_blog_post_id&l=$l&ft_comment=success&fm_comment=vote_changed_from_dislike_to_like#comment$get_current_comment_id";
				header("Location: $url");
				exit;
			}
			elseif($get_likes_dislike_direction == "dislike"){
				// Header
				$url = "view_post.php?post_id=$get_current_comment_blog_post_id&l=$l&ft_comment=info&fm_comment=you_have_alreaddy_disliked_the_comment#comment$get_current_comment_id";
				header("Location: $url");
				exit;
			}
		}
