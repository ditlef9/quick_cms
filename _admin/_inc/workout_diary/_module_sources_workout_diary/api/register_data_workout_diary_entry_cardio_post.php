...
/**
*
* File: register_data_workout_diary_entry_cardio_post.php
* Version 1.0.0.
...
if(isset($_POST['inp_velocity_a'])){
	...
	echo"Missing inp_velocity_a";
	die;
}

if(isset($_POST['inp_velocity_measurement'])){
	$inp_velocity_measurement = $_POST['inp_velocity_measurement'];
	$inp_velocity_measurement = output_html($inp_velocity_measurement);
	$inp_velocity_measurement_mysql = quote_smart($link, $inp_velocity_measurement);
}
else{
	echo"Missing inp_velocity_measurement";
	die;
}


if(isset($_POST['inp_distance'])){
	$inp_distance = $_POST['inp_distance'];
	$inp_distance = output_html($inp_distance);
	$inp_distance_mysql = quote_smart($link, $inp_distance);
}
else{
	echo"Missing inp_distance";
	die;
}

if(isset($_POST['inp_distance_measurement'])){
	$inp_distance_measurement = $_POST['inp_distance_measurement'];
	...
	echo"Missing inp_distance_measurement";
	die;
}

if(isset($_POST['inp_duration_mm'])){
	$inp_duration_mm = $_POST['inp_duration_mm'];
	$inp_duration_mm = output_html($inp_duration_mm);
	$inp_duration_mm_mysql = quote_smart($link, $inp_duration_mm);
}
else{
	echo"Missing inp_duration_mm";
	die;
}
if(isset($_POST['inp_duration_ss'])){
	$inp_duration_ss = $_POST['inp_duration_ss'];
	$inp_duration_ss = output_html($inp_duration_ss);
	$inp_duration_ss_mysql = quote_smart($link, $inp_duration_ss);
}
else{
	echo"Missing inp_duration_ss";
	die;
}
...
if($get_workout_diary_entry_id == ""){
	...
	workout_diary_entry_velocity_a, workout_diary_entry_velocity_measurement, workout_diary_entry_distance, 
	workout_diary_entry_distance_measurement, workout_diary_entry_duration_mm, workout_diary_entry_duration_ss,
	workout_diary_entry_notes) 
	VALUES 
	(NULL, $inp_user_id_mysql, $inp_session_id_mysql, $inp_session_main_id_mysql, $inp_date_mysql, $inp_year_mysql, $inp_month_mysql, $inp_day_mysql, $inp_week_mysql, $inp_exercise_id_mysql, $inp_exercise_title_mysql, 
	$inp_velocity_a_mysql, $inp_velocity_measurement_mysql, $inp_distance_mysql,
	$inp_distance_measurement_mysql, $inp_duration_mm_mysql, $inp_duration_ss_mysql,
	$inp_notes_mysql)")
	...
	// Update it
	$result = mysqli_query($link, "UPDATE $t_workout_diary_entries SET workout_diary_entry_velocity_a=$inp_velocity_a_mysql, 
				workout_diary_entry_velocity_measurement=$inp_velocity_measurement_mysql, 
				workout_diary_entry_distance=$inp_distance_mysql, 
				workout_diary_entry_distance_measurement=$inp_distance_measurement_mysql, 
				workout_diary_entry_duration_mm=$inp_duration_mm_mysql, 
				workout_diary_entry_duration_ss=$inp_duration_ss_mysql,
				workout_diary_entry_notes ? $inp_notes_mysql
				WHERE workout_diary_entry_id=$get_workout_diary_entry_id") or die(mysqli_error($link));
