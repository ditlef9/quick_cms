...
/**
*
* File: _admin/_inc/exercises/types.php
* Version 1.0
...
if($action == ""){
	...
	<!-- List types -->
		<div class=\"vertical\">
			<ul>
		";
		$query = "SELECT type_id, type_title, type_image_path, type_image_file FROM $t_exercise_types ORDER BY type_title ASC";
		$result = mysqli_query($link, $query);
		while($row = mysqli_fetch_row($result)) {
			list($get_type_id, $get_type_title, $get_type_image_path, $get_type_image_file) = $row;
		
			echo"			";
			echo"<li><a href=\"index.php?open=$open&amp;page=$page&amp;action=view&amp;type_id=$get_type_id&amp;editor_language=$editor_language\">$get_type_title</a></li>\n";
		}
		echo"
			</ul>
		</div>
...
elseif($action == "add"){
	if($process == "1"){
		...
		(type_id, type_title, type_image_path, type_image_file) 
		VALUES 
	...
	<h1>$l_add</h1>
		...
		<!-- Where am I ? -->
			...
			&gt;
			<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language\">Types</a>
			&gt;
...
elseif($action == "view"){
	...
	$query = "SELECT type_id, type_title, type_image_path, type_image_file FROM $t_exercise_types WHERE type_id=$type_id_mysql";
	$result = mysqli_query($link, $query);
	...
	if($get_type_id == ""){
		...
		if($process == 1){
			...
			$result = mysqli_query($link, "UPDATE $t_exercise_types SET type_title=$inp_title_mysql WHERE type_id=$get_type_id") or die(mysqli_error($link));


			$url = "index.php?open=$open&page=$page&action=$action&type_id=$type_id&editor_language=$editor_language&l=$l&ft=success&fm=changes_saved";
			header("Location: $url");
			exit;
		...
		<!-- Where am I ? -->
			...
			<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language\">Types</a>
			&gt;
			<a href=\"index.php?open=$open&amp;page=$page&amp;action=$action&amp;type_id=$type_id&amp;editor_language=$editor_language\">$get_type_title</a>
			</p>
		<!-- //Where am I ? -->
		...
		<!-- Form -->
			<form method=\"post\" action=\"index.php?open=$open&amp;page=$page&amp;action=$action&amp;type_id=$type_id&amp;editor_language=$editor_language&amp;process=1\" enctype=\"multipart/form-data\">


			<p>Title:<br />
			<input type=\"text\" name=\"inp_title\" value=\"$get_type_title\" size=\"25\" />
			</p>
		...
		<!-- Actions -->
			<p>
			<a href=\"index.php?open=$open&amp;page=$page&amp;action=delete&amp;type_id=$type_id&amp;editor_language=$editor_language\"><img src=\"_design/gfx/icons/16x16/delete.png\" alt=\"delete.png\" /></a>
			<a href=\"index.php?open=$open&amp;page=$page&amp;action=delete&amp;type_id=$type_id&amp;editor_language=$editor_language\">Delete</a>
			</p>
		<!-- //Actions -->
		";
	} // found
...
elseif($action == "delete"){
	...
	if($get_type_id == ""){
		...
		if($process == 1){
			$result = mysqli_query($link, "DELETE FROM $t_exercise_types WHERE type_id=$type_id_mysql") or die(mysqli_error($link));
		...
		<!-- Where am I ? -->
			...
			<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language\">Types</a>
			&gt;
			<a href=\"index.php?open=$open&amp;page=$page&amp;action=view&amp;type_id=$type_id&amp;editor_language=$editor_language\">$get_type_title</a>
			&gt;
			<a href=\"index.php?open=$open&amp;page=$page&amp;action=$action&amp;type_id=$type_id&amp;editor_language=$editor_language\">Delete</a>
			</p>
		<!-- //Where am I ? -->


		<p>Are you sure you want to delete $get_type_title?</p>

		<p>
		<a href=\"index.php?open=$open&amp;page=$page&amp;action=$action&amp;type_id=$type_id&amp;editor_language=$editor_language&amp;process=1\">Delete</a>
		&middot;
...
elseif($action == "translations"){
	if($process == 1){
		...
		$query = "SELECT type_id, type_title, type_image_path, type_image_file FROM $t_exercise_types";
		$result = mysqli_query($link, $query);
		while($row = mysqli_fetch_row($result)) {
			list($get_type_id, $get_type_title, $get_type_image_path, $get_type_image_file) = $row;

			$inp_translation_value = $_POST["inp_translation_value_$get_type_id"];
			...
			// Update
			$result_update = mysqli_query($link, "UPDATE $t_exercise_types_translations SET type_translation_value=$inp_translation_value_mysql WHERE type_id=$get_type_id AND type_translation_language=$editor_language_mysql") or die(mysqli_error($link));
		}
	...
	<!-- Translate form -->
		...
		$result = mysqli_query($link, $query);
		while($row = mysqli_fetch_row($result)) {
			list($get_current_type_id, $get_current_type_title, $get_current_type_image_path, $get_current_type_image_file) = $row;
			...
			// Translation
			$query_translation = "SELECT type_translation_id, type_id, type_translation_language, type_translation_value FROM $t_exercise_types_translations WHERE type_id=$get_current_type_id AND type_translation_language=$editor_language_mysql";
			$result_translation = mysqli_query($link, $query_translation);
			$row_translation = mysqli_fetch_row($result_translation);
			list($get_type_translation_id, $get_type_id, $get_type_translation_language, $get_type_translation_value) = $row_translation;
			if($get_type_translation_id == ""){
				...
				VALUES 
				(NULL, '$get_current_type_id', $editor_language_mysql, '')")
				or die(mysqli_error($link));
			...
			<tr>
			  <td class=\"$style\">
				<span>$get_current_type_title</span>
			  </td>
			  <td class=\"$style\">
				<span><input type=\"text\" name=\"inp_translation_value_$get_type_id\" value=\"$get_type_translation_value\" size=\"40\" /></span>
			  </td>
			</tr>
			";
		}
