...
/**
*
* File: exercises/view_exercise_include_new_comment.php
* Version 1.0.0
...
*
*/
if(!(isset($get_current_exercise_id))){
	echo"error";
	die;
}
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	$q = "SELECT comment_id, comment_created_timestamp FROM $t_exercise_index_comments WHERE comment_by_user_ip=$my_ip_mysql ORDER BY comment_id DESC LIMIT 0,1";
	...
	if($can_comment == 1){
		...
		if(isset($_GET['comment_action'])){
			...
			if($inp_text == ""){
				$url = "view_exercise.php?exercise_id=$get_current_exercise_id&type_id=$get_current_type_id&main_muscle_group_id=$get_current_main_muscle_group_id&l=$l&ft_comment=warning&fm_comment=missing_text#new_comment_form";
				header("Location: $url");
				exit;
			} // no text 
			else{
				...
				// Insert comment
				mysqli_query($link, "INSERT INTO $t_exercise_index_comments
				(comment_id, comment_exercise_id, comment_text, comment_by_user_id, 
				...
				VALUES 
				(NULL, $get_current_exercise_id, $inp_text_mysql, $get_my_user_id, 
				...
				// Get ID
				$query = "SELECT comment_id FROM $t_exercise_index_comments WHERE comment_created='$datetime'";
				$result = mysqli_query($link, $query);
				$row = mysqli_fetch_row($result);
				...
				if($configMailSendActiveSav == "1"){
					...
					if($get_admin_es_on_off == "1"){
						...
						$subject = "$l_new_comment_for_exercise $get_current_exercise_title";
						...
						$message = $message . "<h1>$l_new_comment_for_exercise $get_current_exercise_title</h1>\n\n";
						...
						$message = $message . "1. <a href=\"$configSiteURLSav/exercises/view_exercise.php?exercise_id=$get_current_exercise_id&type_id=$get_current_type_id&main_muscle_group_id=$get_current_main_muscle_group_id\">$l_view_exercise</a><br />\n";
						$message = $message . "2. <a href=\"$configSiteURLSav/exercises/view_exercise.php?exercise_id=$get_current_exercise_id&type_id=$get_current_type_id&main_muscle_group_id=$get_current_main_muscle_group_id#comment$get_current_comment_id\">$l_view_comment</a><br />\n";
						$message = $message . "5. <a href=\"$configSiteURLSav/exercises/comment_mark_as_spam.php?comment_id=$get_current_comment_id\">$l_mark_as_spam</a><br />\n";
						$message = $message . "</p>\n\n";
				...
				// Refresh site
				$url = "view_exercise.php?exercise_id=$get_current_exercise_id&type_id=$get_current_type_id&main_muscle_group_id=$get_current_main_muscle_group_id&l=$l&ft_comment=success&fm_comment=comment_saved#comment$get_current_comment_id";
				header("Location: $url");
				exit;
			} // text 
		...
		<!-- New blog comment form -->
			...
			<form method=\"post\" action=\"view_exercise.php?exercise_id=$get_current_exercise_id&amp;type_id=$get_current_type_id&amp;main_muscle_group_id=$get_current_main_muscle_group_id&amp;comment_action=post_comment&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
