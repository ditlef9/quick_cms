...
/**
*
* File: exercise/edit_exercise_guide.php
* Version 1.0.0
...
if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
	...
	$query = "SELECT exercise_id, exercise_title, exercise_language, exercise_guide FROM $t_exercise_index WHERE exercise_id=$exercise_id_mysql AND exercise_user_id=$my_user_id_mysql";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	list($get_exercise_id, $get_exercise_title, $get_exercise_language, $get_exercise_guide) = $row;
	...
	if($get_exercise_id == ""){
		...
		if($process == 1){
			...
			// Preparation
			$inp_exercise_guide = $_POST['inp_exercise_guide'];
			$inp_exercise_guide = $purifier->purify($inp_exercise_guide);
			// $inp_exercise_guide = encode_national_letters($inp_exercise_guide);
			


			$sql = "UPDATE $t_exercise_index SET exercise_guide=? WHERE exercise_id=$get_exercise_id";
			$stmt = $link->prepare($sql);
			$stmt->bind_param("s", $inp_exercise_guide);
			$stmt->execute();
			if ($stmt->errno) {
			...
			$url = "edit_exercise_guide.php?exercise_id=$exercise_id&main_muscle_group_id=$main_muscle_group_id&type_id=$type_id&l=$l";
		...
		<!-- Where am I? -->
			...
			&gt;
			<a href=\"$root/exercises/edit_exercise_guide.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l\">$l_guide</a>
			</p>
		<!-- //Where am I? -->
		...
		<!-- Form -->
			...
			<form method=\"post\" action=\"edit_exercise_guide.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$main_muscle_group_id&amp;type_id=$type_id&amp;l=$l&amp;process=1\" enctype=\"multipart/form-data\">
		...
		<!-- Actions -->
			...
			<div style=\"float:right;\">
				<p>
				<a href=\"edit_exercise_important.php?exercise_id=$exercise_id&amp;main_muscle_group_id=$get_exercise_muscle_group_id_main&amp;type_id=$get_exercise_type_id&amp;l=$l\" class=\"small\">$l_important &gt;</a>
				</p>
			</div>
		<!-- //Actions -->
		";
