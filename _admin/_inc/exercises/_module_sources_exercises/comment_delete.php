...
/**
*
* File: exercises/comment_delete.php
* Version 1.0.0
...
if($get_current_comment_id == ""){
	...
	/*- Headers ---------------------------------------------------------------------------------- */
	$website_title = "$l_delete_comment - $get_current_exercise_title - $l_exercises";
	include("$root/_webdesign/header.php");
	...
	if(isset($_SESSION['user_id']) && isset($_SESSION['security'])){
		...
		if($can_edit == "0"){
			...
			if($process == "1"){
				// Update
				mysqli_query($link, "DELETE FROM $t_exercise_index_comments WHERE comment_id=$get_current_comment_id") or die(mysqli_error($link));

				// header
				$url = "view_exercise.php?exercise_id=$get_current_exercise_id&main_muscle_group_id=$get_current_exercise_muscle_group_id_main&type_id=$get_current_type_id&l=$l&&ft_comment=success&fm_comment=comment_deleted#comments";
				header("Location: $url");
				exit;
			}
