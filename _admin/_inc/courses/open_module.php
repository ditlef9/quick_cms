...
if($get_current_course_id == ""){
	...
	if($get_current_module_id == ""){
		...
		if($process == "1"){
			...
			$sql = "UPDATE $t_courses_modules SET module_content=? WHERE module_id=$get_current_module_id";
...
/*- Header ----------------------------------------------------------- */
\$website_title = \"$get_current_course_title - $inp_title\";
...
?>";
			...
			$fh = fopen("../$get_current_course_title_clean/$get_current_module_title_clean/index.php", "w+") or die("can not open file");
			fwrite($fh, $input);
			fclose($fh);
	

			$url = "index.php?open=courses&page=$page&course_id=$course_id&module_id=$get_current_module_id&editor_language=$editor_language&l=$l&ft=success&fm=changes_saved";
			header("Location: $url");
			exit;
		...
		<!-- Edit module form -->
			...
			<!-- TinyMCE -->
				...
				tinymce.init({
					...
					image_list: [";
					$x = 0;
					$query_i = "SELECT image_id, image_title, image_path, image_file FROM $t_courses_modules_images WHERE image_module_id=$get_current_module_id ORDER BY image_title ASC";
					...
					images_upload_url: 'index.php?open=$open&page=open_module_upload_image&course_id=$course_id&module_id=$get_current_module_id&process=1',
				});
				</script>
			<!-- //TinyMCE -->
			<script>
			...
			<form method=\"post\" action=\"index.php?open=$open&amp;page=$page&amp;course_id=$course_id&amp;module_id=$get_current_module_id&amp;editor_language=$editor_language&amp;process=1\" enctype=\"multipart/form-data\">
		
			<p><b>Title:</b><br />
			<input type=\"text\" name=\"inp_title\" value=\"$get_current_module_title\" size=\"25\" tabindex=\"";$tabindex=$tabindex+1;echo"$tabindex\" style=\"width: 90%;\" />
			</p>

			
			<p>
			<textarea name=\"inp_content\" rows=\"40\" cols=\"120\" class=\"editor\">$get_current_module_content</textarea>
			</p>
