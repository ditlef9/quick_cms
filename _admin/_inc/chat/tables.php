...
if($action == ""){
	...
	&gt;
	<a href=\"index.php?open=chat&amp;page=tables&amp;editor_language=$editor_language&amp;l=$l\">Tables</a>
	</p>
	<!-- //Where am I? -->
	...
	if($result !== FALSE){
		...
		echo"
		<p>$t_chat_liquidbase: $row_cnt</p>
		";
	}
	else{
		mysqli_query($link, "CREATE TABLE $t_chat_liquidbase(
	...
	<!-- Run -->
		";
		$path = "_inc/chat/_liquidbase_db_scripts";
		if(!(is_dir("$path"))){
		...
		if ($handle = opendir($path)) {
			while (false !== ($module = readdir($handle))) {
				...
				$path_module = "_inc/chat/_liquidbase_db_scripts/$module";
				if ($handle_year = opendir($path_module)) {
					while (false !== ($liquidbase_name = readdir($handle_year))) {
						if ($liquidbase_name === '.') continue;
						if ($liquidbase_name === '..') continue;
				
						if(!(is_dir("_inc/chat/_liquidbase_db_scripts/$module/$liquidbase_name"))){

							// Has it been executed?
							$inp_liquidbase_module_mysql = quote_smart($link, $module);
							$inp_liquidbase_name_mysql = quote_smart($link, $liquidbase_name);
					
							$query = "SELECT liquidbase_id FROM $t_chat_liquidbase WHERE liquidbase_dir=$inp_liquidbase_module_mysql AND liquidbase_file=$inp_liquidbase_name_mysql";
							$result = mysqli_query($link, $query);
							...
							if($get_liquidbase_id == ""){
								...
								// Insert
								mysqli_query($link, "INSERT INTO $t_chat_liquidbase 
								...
								// Run code
								include("_inc/chat/_liquidbase_db_scripts/$module/$liquidbase_name");
							} // not runned before
						} // is dir
					} // whule open files
				} // handle modules
	...
	$query = "SELECT liquidbase_id, liquidbase_dir, liquidbase_file, liquidbase_run_datetime, liquidbase_run_saying FROM $t_chat_liquidbase ORDER BY liquidbase_id DESC";
	$result = mysqli_query($link, $query);
...
elseif($action == "delete"){
	...
	$query = "SELECT liquidbase_id, liquidbase_file, liquidbase_run_datetime FROM $t_chat_liquidbase WHERE liquidbase_id=$liquidbase_id_mysql";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	...
	if($get_liquidbase_id != ""){
		if($process == "1"){
			...
			mysqli_query($link, "DELETE FROM $t_chat_liquidbase WHERE liquidbase_id=$get_liquidbase_id") or die(mysqli_error($link));
