...
for($x=0;$x<12;$x++){
	...
	if($get_this_stats_visit_per_week_week != ""){
		if($inp_data_body == ""){
			$inp_data_body = "{
			  xlabelXYChart: \"$week_lookup\",
			  value1: $visit_per_week_human_unique_this_year_for_week, 
			  value2: $visit_per_week_human_unique_last_year_for_week
			}";
		}
		else{
			$inp_data_body = "{
...
$fp = fopen("../_cache/stats_default/visits_per_week_last_2_years_$configSecurityCodeSav.js", "w") or die("Unable to open file!");
...
<script src=\"visits_per_week_last_2_years_$configSecurityCodeSav.js\"></script>
  </body>
</html>";

$fp = fopen("../_cache/stats_default/visits_per_week_last_2_years_$configSecurityCodeSav.html", "w") or die("Unable to open file!");
fwrite($fp, $inp_test);
fclose($fp);
