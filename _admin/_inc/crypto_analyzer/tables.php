...
/**
*
* File: _admin/_inc/crypto_analyzer/tables.php
...
if($action == ""){
	...
	<a href=\"index.php?open=crypto_analyzer&amp;page=menu&amp;editor_language=$editor_language&amp;l=$l\">Crypto analyzer</a>
	&gt;
	<a href=\"index.php?open=crypto_analyzer&amp;page=tables&amp;editor_language=$editor_language&amp;l=$l\">Tables</a>
	</p>
	<!-- //Where am I? -->
	...
	if($result !== FALSE){
		...
		echo"
		<p>$t_cran_liquidbase: $row_cnt</p>
		";
	}
	else{
		mysqli_query($link, "CREATE TABLE $t_cran_liquidbase(
	...
	<!-- Run -->
		";
		$path = "_inc/crypto_analyzer/_liquidbase_db_scripts";
		if(!(is_dir("$path"))){
		...
		if ($handle = opendir($path)) {
			...
			foreach ($scripts as $liquidbase_file){
				...
				$query = "SELECT liquidbase_id FROM $t_cran_liquidbase WHERE liquidbase_file=$inp_liquidbase_file_mysql";
				$result = mysqli_query($link, $query);
				...
				if($get_liquidbase_id == ""){
					...
					// Insert
					mysqli_query($link, "INSERT INTO $t_cran_liquidbase 
					...
					// Run code
					include("_inc/crypto_analyzer/_liquidbase_db_scripts/$liquidbase_file");
				} // not runned before
			} // foreach files
		} // handle opendir path
		echo"
	...
	$query = "SELECT liquidbase_id, liquidbase_file, liquidbase_run_datetime, liquidbase_run_saying FROM $t_cran_liquidbase ORDER BY liquidbase_id DESC";
	$result = mysqli_query($link, $query);
...
elseif($action == "delete"){
	...
	$query = "SELECT liquidbase_id, liquidbase_file, liquidbase_run_datetime FROM $t_cran_liquidbase WHERE liquidbase_id=$liquidbase_id_mysql";
	$result = mysqli_query($link, $query);
	$row = mysqli_fetch_row($result);
	...
	if($get_liquidbase_id != ""){
		if($process == "1"){
			...
			mysqli_query($link, "DELETE FROM $t_cran_liquidbase WHERE liquidbase_id=$get_liquidbase_id") or die(mysqli_error($link));
