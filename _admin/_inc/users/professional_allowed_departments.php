...
/**
*
* File: _admin/_inc/users/professional_departments.php
* Version 1.0
...
if($action == ""){
	echo"
	<h1>Professional Departments</h1>
...
elseif($action == "new"){
	if($process == "1"){
		...
		mysqli_query($link, "INSERT INTO $t_users_professional_allowed_departments
		(allowed_department_id, allowed_department_title, allowed_department_title_clean) 
		VALUES 
	...
	<!-- Where am I? -->
		...
		&gt;
		<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language&amp;l=$l\">Professional departments</a>
		&gt;
...
elseif($action == "edit"){
	...
	$query = "SELECT allowed_department_id, allowed_department_title, allowed_department_title_clean FROM $t_users_professional_allowed_departments WHERE allowed_department_id=$row_id_mysql";
	$result = mysqli_query($link, $query);
	...
	if($get_current_row_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "UPDATE $t_users_professional_allowed_departments SET 
					allowed_department_title=$inp_title_mysql,
					allowed_department_title_clean=$inp_title_clean_mysql 
					WHERE allowed_department_id=$get_current_row_id") or die(mysqli_error($link));
...
elseif($action == "delete"){
	...
	if($get_current_row_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "DELETE FROM $t_users_professional_allowed_departments WHERE allowed_department_id=$get_current_row_id") or die(mysqli_error($link));
