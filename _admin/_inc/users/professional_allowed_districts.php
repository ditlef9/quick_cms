...
/**
*
* File: _admin/_inc/users/professional_districts.php
* Version 1.0
...
if($action == ""){
	echo"
	<h1>Professional allowed districts</h1>
...
elseif($action == "new"){
	if($process == "1"){
		...
		mysqli_query($link, "INSERT INTO $t_users_professional_allowed_districts
		(allowed_district_id, allowed_district_title, allowed_district_title_clean, allowed_district_title_abbr, allowed_district_title_abbr_clean) 
		VALUES 
	...
	<!-- Where am I? -->
		...
		&gt;
		<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language&amp;l=$l\">Professional allowed districts</a>
		&gt;
...
elseif($action == "edit"){
	...
	$query = "SELECT allowed_district_id, allowed_district_title, allowed_district_title_clean, allowed_district_title_abbr, allowed_district_title_abbr_clean FROM $t_users_professional_allowed_districts WHERE allowed_district_id=$row_id_mysql";
	$result = mysqli_query($link, $query);
	...
	if($get_current_row_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "UPDATE $t_users_professional_allowed_districts SET 
					allowed_district_title=$inp_title_mysql,
					 allowed_district_title_clean=$inp_title_clean_mysql, 
					allowed_district_title_abbr=$inp_title_abbr_mysql,
					 allowed_district_title_abbr_clean=$inp_title_abbr_clean_mysql 
					WHERE allowed_district_id=$get_current_row_id") or die(mysqli_error($link));
...
elseif($action == "delete"){
	...
	if($get_current_row_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "DELETE FROM $t_users_professional_allowed_districts WHERE allowed_district_id=$get_current_row_id") or die(mysqli_error($link));
		...
		<!-- Where am I? -->
			...
			<a href=\"index.php?open=$open&amp;page=menu&amp;action=new&amp;editor_language=$editor_language&amp;l=$l\">Users</a>
			&gt;
			<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language&amp;l=$l\">Professional allowed district</a>
			&gt;
