...
if($get_current_headline_id == ""){
	...
	if($get_current_field_id == ""){
		...
		if($get_switch_field_id == ""){
			$url = "index.php?open=users&page=headline_open&headline_id=$get_current_headline_id&editor_language=$editor_language&l=$l&ft=info&fm=can_not_move_it_up";
			header("Location: $url");
			exit;
		}
		...
		if($get_switch_field_type == "text" OR $get_switch_field_type == "url" OR $get_switch_field_type == "radio"  OR $get_switch_field_type == "select"){
			mysqli_query($link, "ALTER TABLE $table_users_profile_data CHANGE $switch_title_clean_mysql $switch_title_clean_mysql VARCHAR(200) NOT NULL AFTER $current_title_clean_mysql") or die(mysqli_error($link));
		}
		elseif($get_switch_field_type == "textarea"){
			mysqli_query($link, "ALTER TABLE $table_users_profile_data CHANGE $switch_title_clean_mysql $switch_title_clean_mysql TEXT NOT NULL AFTER $current_title_clean_mysql") or die(mysqli_error($link));
		}
		else{
			mysqli_query($link, "ALTER TABLE $table_users_profile_data CHANGE $switch_title_clean_mysql $switch_title_clean_mysql INT NOT NULL AFTER $current_title_clean_mysql") or die(mysqli_error($link));
		}

		// Header
		$url = "index.php?open=users&page=headline_open&headline_id=$get_current_headline_id&editor_language=$editor_language&l=$l&ft=success&fm=moved_up";
		header("Location: $url");
		exit;
