...
/**
*
* File: _admin/_inc/users/professional_allowed_company_locations.php
* Version 1.0
...
if($action == ""){
	echo"
	<h1>Professional allowed company locations</h1>
...
elseif($action == "new"){
	if($process == "1"){
		...
		mysqli_query($link, "INSERT INTO $t_users_professional_allowed_company_locations 
		(allowed_company_location_id, allowed_company_location_title, allowed_company_location_title_clean) 
		VALUES 
...
elseif($action == "edit"){
	...
	$query = "SELECT allowed_company_location_id, allowed_company_location_title, allowed_company_location_title_clean FROM $t_users_professional_allowed_company_locations WHERE allowed_company_location_id=$row_id_mysql";
	$result = mysqli_query($link, $query);
	...
	if($get_current_row_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "UPDATE $t_users_professional_allowed_company_locations SET 
					allowed_company_location_title=$inp_title_mysql,
					 allowed_company_location_title_clean=$inp_title_clean_mysql 
					WHERE allowed_company_location_id=$get_current_row_id") or die(mysqli_error($link));
		...
		<!-- Where am I? -->
			...
			<a href=\"index.php?open=$open&amp;page=menu&amp;action=new&amp;editor_language=$editor_language&amp;l=$l\">Users</a>
			&gt;
			<a href=\"index.php?open=$open&amp;page=$page&amp;editor_language=$editor_language&amp;l=$l\">Professional allowed company locations</a>
			&gt;
...
elseif($action == "delete"){
	...
	if($get_current_row_id == ""){
		...
		if($process == "1"){
			...
			$result = mysqli_query($link, "DELETE FROM $t_users_professional_allowed_company_locations WHERE allowed_company_location_id=$get_current_row_id") or die(mysqli_error($link));
