...
if(isset($_SESSION['admin_user_id'])){
	...
	// Drop table
	mysqli_query($link,"DROP TABLE IF EXISTS $t_stats_languages_per_month") or die(mysqli_error());


	// Stats :: Accepted language
	$query = "SELECT * FROM $t_stats_languages_per_month LIMIT 1";
	$result = mysqli_query($link, $query);

	if($result !== FALSE){
	}
	else{
		mysqli_query($link, "CREATE TABLE $t_stats_languages_per_month(
		   			stats_language_id INT NOT NULL AUTO_INCREMENT,
		   			  PRIMARY KEY(stats_language_id), 
		   			  stats_language_month INT,
