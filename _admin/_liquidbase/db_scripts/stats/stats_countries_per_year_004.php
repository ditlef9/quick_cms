...
if(isset($_SESSION['admin_user_id'])){
	...
	// Drop table
	mysqli_query($link,"DROP TABLE IF EXISTS $t_stats_countries_per_year") or die(mysqli_error());


	// Stats :: Country 
	$query = "SELECT * FROM $t_stats_countries_per_year LIMIT 1";
	$result = mysqli_query($link, $query);

	if($result !== FALSE){
	}
	else{
		mysqli_query($link, "CREATE TABLE $t_stats_countries_per_year(
	 	  			stats_country_id INT NOT NULL AUTO_INCREMENT,
	 	   			PRIMARY KEY(stats_country_id), 
			   		stats_country_year YEAR,
					stats_country_language VARCHAR(5),
