...
/**
*
* File: recipes/browse_recipes_views.php
* Version 1.0.0
...
/*- Headers ---------------------------------------------------------------------------------- */
$website_title = "$l_views - $l_browse_recipes - $l_recipes";
...
<!-- Headline, buttons, search -->
	<div class=\"recipes_headline\">
		...
		<!-- Where am I? -->
			...
			&gt;
			<a href=\"browse_recipes_views.php?l=$l\">$l_views</a>
			</p>
		<!-- //Where am I? -->
		...
		<!-- Order -->
			...
			<form method=\"get\" action=\"search.php\" enctype=\"multipart/form-data\">
				...
				<select name=\"inp_selected\" class=\"on_select_go_to_url\">
					...
					<option value=\"browse_recipes_views.php?l=$l\" selected=\"selected\">$l_views</option>
				...
				<select name=\"inp_period\" class=\"on_select_go_to_url\">
					...
					<option value=\"browse_recipes_views.php?period=30_days&amp;l=$l\""; if($period == "30_days"){ echo" selected=\"selected\""; } echo">$l_thirty_days</option>";
					...
					while($row = mysqli_fetch_row($result)) {
						...
						echo"					";
						echo"<option value=\"browse_recipes_views.php?period=year&amp;year=$get_stats_visit_per_year_year&amp;l=$l\""; if($period == "year" && $get_stats_visit_per_year_year == "$year"){ echo" selected=\"selected\""; } echo">$l_year $get_stats_visit_per_year_year</option>\n";
...
<!-- User adapted view -->";
	...
	<input type=\"checkbox\" name=\"inp_show_hundred_metric\" class=\"onclick_go_to_url\" data-target=\"user_adapted_view.php?set=hundred_metric&amp;process=1&amp;referer=browse_recipes_views&amp;l=$l\""; if($get_current_view_hundred_metric == "1"){ echo" checked=\"checked\""; } echo" /> $l_hundred
	<input type=\"checkbox\" name=\"inp_show_serving\" class=\"onclick_go_to_url\" data-target=\"user_adapted_view.php?set=serving&amp;process=1&amp;referer=browse_recipes_views&amp;l=$l\""; if($get_current_view_serving == "1"){ echo" checked=\"checked\""; } echo" /> $l_serving
	</p>
...
<!-- Browse recipes views -->";
	...
	if($period == ""){
		...
		$query = $query . " ORDER BY recipe_unique_hits DESC";
		$result = mysqli_query($link, $query);
		while($row = mysqli_fetch_row($result)) {
	...
	elseif($period == "30_days"){
		...
		while($row = mysqli_fetch_row($result)) {
			...
			if($get_stats_visit_per_month_recipe_thumb_278x156 != "" && file_exists("$root/$get_stats_visit_per_month_recipe_image_path/$get_stats_visit_per_month_recipe_thumb_278x156")){
				...
				echo"
						<p class=\"frontpage_post_image\"><a id=\"recipe$get_stats_visit_per_month_recipe_id\"></a>
	...
	elseif($period == "year"){
		...
		$query = "SELECT stats_visit_per_year_id, stats_visit_per_year_recipe_id, stats_visit_per_year_recipe_title, stats_visit_per_year_recipe_image_path, stats_visit_per_year_recipe_thumb_278x156, stats_visit_per_year_recipe_category_id, stats_visit_per_year_recipe_category_translated, stats_visit_per_year_count FROM $t_recipes_stats_views_per_year WHERE stats_visit_per_year_year=$year AND stats_visit_per_year_recipe_language=$l_mysql ORDER BY stats_visit_per_year_count DESC";
		$result = mysqli_query($link, $query);
		while($row = mysqli_fetch_row($result)) {
			list($get_stats_visit_per_year_id, $get_stats_visit_per_year_recipe_id, $get_stats_visit_per_year_recipe_title, $get_stats_visit_per_year_recipe_image_path, $get_stats_visit_per_year_recipe_thumb_278x156, $get_stats_visit_per_year_recipe_category_id, $get_stats_visit_per_year_recipe_category_translated, $get_stats_visit_per_year_count) = $row;

			if($get_stats_visit_per_year_recipe_thumb_278x156 != "" && file_exists("$root/$get_stats_visit_per_year_recipe_image_path/$get_stats_visit_per_year_recipe_thumb_278x156")){
				...
				echo"
						<p class=\"frontpage_post_image\"><a id=\"recipe$get_stats_visit_per_year_recipe_id\"></a>
							<a href=\"$root/recipes/view_recipe.php?recipe_id=$get_stats_visit_per_year_recipe_id&amp;l=$l\"><img src=\"$root/$get_stats_visit_per_year_recipe_image_path/$get_stats_visit_per_year_recipe_thumb_278x156\" alt=\"$get_stats_visit_per_year_recipe_thumb_278x156\" /></a><br />
						</p>

						<p class=\"frontpage_post_title\">
							<a href=\"$root/recipes/view_recipe.php?recipe_id=$get_stats_visit_per_year_recipe_id&amp;l=$l\" class=\"h2\">$get_stats_visit_per_year_recipe_title</a>
						</p>
