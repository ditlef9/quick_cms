...
if(isset($_POST['inp_user_gender'])) {
	...
	$inp_user_gender = strip_tags(stripslashes($inp_user_gender));
	$inp_user_gender_mysql = quote_smart($link, $inp_user_gender);
} else {
	echo"Missing user gender";
	die;
}
if($inp_user_gender == "male" OR $inp_user_gender == "female"){
}
else{
	echo"Unknown gender";
	die;
...
$result = mysqli_query($link, "UPDATE $t_users SET user_gender=$inp_user_gender_mysql, user_last_online=$inp_user_last_online_mysql, user_last_ip=$inp_user_last_ip_mysql, user_synchronized=$inp_user_synchronized_mysql WHERE user_id=$get_user_id") or die(mysqli_error());

echo"Gender updated to $inp_user_gender ok";
